...
// it defaults to "controller mode" again see i2c.rs, 

//   let mut i2c = hal::I2C::i2c1(
// use hal::eh1_0_alpha::I2c as eh1;
//    let mut i2c = eh1::i2c1(   // trying to use the alpha version of funcs
// let mut i2c = hal::BlockingI2c::i2c1(
...
// Write three bytes to the I²C device with 7-bit address 0x00
    ...
    loop {
        for idx in 0..SIN_TBL_SIZ {
		// i2c_load[0] = (((sin_table[idx] / 256) as u8) & 0x0f); // upper 4 bits of 12 bit word
		// i2c_load[1] = ((sin_table[idx] % 256) as u8); // lower 8 bits of 12 bit word

		i2c_load[0] = 0x40; // command write 	
		i2c_load[1] = (((sin_table[idx] / 16) as u8) ); // upper 8 bits of 12 bit word
		i2c_load[2] = ((sin_table[idx] % 16) as u8 ) << 4; // lower 4 bits of 12 bit word
		for _ in 0..2 {
		   let res=i2c.write(0x60, &i2c_load[0..2]); //why 63 and not 61 or 60 who knows, I do have a0 tied to ADC so that should be a
		   // let res= i2c.write_iter(0x61, &i2c_load[0..2]); // try iterative writing
		   let res = match res  {
            ...
            // there are plenty of abort reasons in controller.rs we may want to look at.
	    	...
	    	delay.delay_ms(1); // delay 2.5 seconds so we can check the voltage on the DAC out
